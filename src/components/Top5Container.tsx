      ...
      <ul>
        {items.map(({ name, image, artists, url }, index: number) => (
          <li
            key={index}
          >
            <Link href={url} className='flex items-center gap-4 rounded-xl px-2 cursor-pointer text-white no-underline hover:text-lime-500 hover:no-underline md:gap-6 py-3 md:py-4'>
